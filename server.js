...
let pendingUnknownId = 0; // last USER message id that should trigger a short reaction
let handledUnknownId = 0; // last USER message id already handled (one-time)
...
— если в истории появляется строка "X: ...", считай это неизвестным говорящим.
— можно кратко удивиться/проверить "кто это" ОДИН РАЗ, и дальше НЕ строить весь диалог вокруг этого.

ФОРМАТ:
— 1 короткая реплика, 1–3 предложения.
— НЕ используй списки.
...
function cleanText(text) {
  return String(text || "")
    ...
    // ✅ kill leading bullet/hyphen the model sometimes adds
    .replace(/^\s*[-—–]\s+/, "")
    .trim();
}
...
async function generateNext() {
  ...
  // ✅ one-time nudge only for BASMATI (speaker A) and only once per new X
  const shouldReactOnce = speaker === "A" && pendingUnknownId > handledUnknownId;

  const oneTimeNudge = shouldReactOnce
    ? `\n\nВнимание: в истории есть свежая реплика X (неизвестный голос). В ЭТОЙ реплике коротко отреагируй (1 фразой удивления/вопроса), и затем сразу продолжи обычный разговор. Не зацикливайся и не возвращайся к этому в следующих репликах.`
    : "";
...
Запрещено: списки, лекции, длинные монологи.
...
— можно кратко удивиться/уточнить ОДИН РАЗ, но дальше разговор идёт своим ходом и НЕ крутится вокруг X
...
Цель: живой разговор.`,
  ...
  // ✅ mark unknown voice handled after BASMATI's one-time reaction
...
app.post("/say", async (req, res) => {
  ...
  // ✅ make this X "pending", so BASMATI reacts once (next A reply only)
  ...
  try {
    ...
    lastSpeaker = "B";
    const r1 = await generateNext(); // BASMATI reacts (one-time if X pending)
    const r2 = await generateNext(); // then KUBANSKIY continues normally
